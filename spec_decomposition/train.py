...
def train_step(state, inputs, outputs, targets, learning_rate_fn, config, dropout_rng):
    """Train on a batch of data."""
    # Split the PRNG key for dropout.
    dropout_rng, new_dropout_rng = jax.random.split(dropout_rng)

    weights = jnp.where(targets > 0, 1, 0).astype(jnp.float32)

    # Define a loss function that computes the loss and model output.
    def loss_fn(params):
    ...
    # Apply the gradient updates to the state.
...
def eval_step(state, inputs, outputs, targets, eos_token, config):
    ...
    if not FLAGS.predict_only:
      ...
      if (step and step % FLAGS.eval_freq == 0) or is_last_step:
        ...
        for batches in eval_ds.as_numpy_iterator():
          ...
          metrics = p_eval_step(state, inputs, outputs, targets)
      ...
      for dataset, predict_or_test in [(predict_ds, 'predict'),
        ...
        for beam_size in [1, 10]:
          ...
          for batches in dataset.as_numpy_iterator():
            ...
            predicted = p_pred_step(state, inputs, outputs, cache,
                                    beam_size)
