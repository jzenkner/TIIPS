...
# base_data_dir=./tiips/test_data_tf_records/deepcoder
...
# train_run=1
# base_model_dir=./tiips/trained_models/deepcoder

num_test=500
eval_run=e2e_predict_1
save_dir=./tiips_results/evaluation/deepcoder_${eval_run}
adr=0.1,ara=True,dr=0.1,e=NONE,ed=360,hd=720,l=0.0002,md=60,mpced=180,npb=32,s=10,scnpr=0.0,ura=True

test_dataset_format=${base_data_dir}/{experiment}_data/entire_programs_test.tf_records*
spec_decomposer_path_format=${base_model_dir}/spec_decomposer_model/checkpoints/adr=0.1,ara=True,dr=0.1,e={experiment},ed=${embedding_dim},hd=${hidden_dim},l=0.0002,md=60,mpced=180,npb=32,s={seed},scnpr=0.0,ura=True/
...
for prediction_type in baseline; do

  python -m spec_decomposition.launch_end_to_end_predict \
  --exp_title=end_to_end_predict-deepcoder-run-${eval_run}-${prediction_type}-${step_limit} \
  --save_dir=${save_dir} \
