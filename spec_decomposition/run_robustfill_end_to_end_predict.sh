...
# base_data_dir=./tiips_data/robustfill_data
...
# train_run=1
# base_model_dir=./tiips_results/exedec_train_robustfill_run-${train_run}

num_test=1000
eval_run=e2e_predict_1
save_dir=./tiips_results/evaluation/robustfill_${eval_run}
...
for prediction_type in tiips separate baseline; do
  ...
  if [[ "${prediction_type}" in "separate" "tiips" ]]; then
    max_io_length=200
