...
def load_spec_decomposer_model(init_rng, spec_vocab_size, io_shape,
  ...
  # Restore the checkpoint (which was saved using flax.optim)
      old_optimizer = checkpoints.restore_checkpoint(format_path(_SPEC_DECOMPOSER_PATH_FORMAT.value), target = None)  # Specify None to get the full optimizer
      ...
      state = checkpoints.restore_checkpoint(format_path(_SPEC_DECOMPOSER_PATH_FORMAT.value), state)
...
def load_synthesizer_model(init_rng, spec_vocab_size, program_vocab_size,
  ...
  if "gs://" in format_path(_SYNTHESIZER_PATH_FORMAT.value):
      # Restore the checkpoint (which was saved using flax.optim)
      old_optimizer = checkpoints.restore_checkpoint(format_path(_SYNTHESIZER_PATH_FORMAT.value),
      ...
      state = checkpoints.restore_checkpoint(format_path(_SYNTHESIZER_PATH_FORMAT.value), state)
...
def main(_):
    ...
    for i in inputs:
      if program == '[invalid program]':
        ...
        try:
          ...
          # print(e)
          # print(program, inputs, step_index)
          outputs.append(default_output)
  ...
  for test_example_index, batch in enumerate(test_dataset.as_numpy_iterator()):
    ...
    while jnp.any(~outer_aux['finished']) and outer_step_index < outer_step_limit:
        ...
        ground_truth_output_parts = [
            deepcoder_dsl.result_to_str(
                ground_truth.run(inputs_program_state.state).get_index(
                    outer_step_index + deepcoder_num_inputs))
            for inputs_program_state in inputs_program_states
        ]
      else:
        ground_truth_program_part = ground_truth.expressions[outer_step_index]
        ground_truth_output_parts = [ground_truth_program_part(i)
                                     for i in decoded_inputs]

      # Run spec decomposition step for ExeDec
      if pred_type == 'separate': #  or (pred_type == 'tiips' and outer_step_index > 0):
        ...
        predicted_spec_parts, scores, aux = spec_decomposer_pred_step(  # pylint: disable=undefined-variable
            ...
            inputs=outer_aux['current_inputs'],
            outputs=outer_aux['current_outputs'],
            cache=None,
            aux=outer_aux,
            beam_size=beam_size)
        ...
        if do_logging:
          ...
          log_message += '\n' + ('=' * 80) + '\n'
          log_message += (
              f'Spec Decomposition Step #{outer_step_index + 1}:\n'
        ...
        if jnp.all(aux['finished']):
          outer_step_index += 1  # Count this half-step.
          break
      else:
        aux = copy.deepcopy(outer_aux)
    
      inner_step_index = 0
      # For TIIPS, make sure the inner loop has only max_steps - guidance_steps iterations to generate a program without guidance
      inner_step_limit = step_limit - outer_step_index if pred_type == 'tiips' else 1
      # Inductive inner loop
      while jnp.any(~aux['finished']) and inner_step_index < inner_step_limit:
        # Synthesizer Step.
        # Run the SynthesizerModel.
        start_time = timeit.default_timer()
        if pred_type == 'separate':
        ...
        var_idx = inner_step_index + deepcoder_num_inputs if pred_type == 'tiips' else outer_step_index + inner_step_index + deepcoder_num_inputs
        ...
        for i in range(beam_size):
          ...
          # or already invalid.
          if (not aux['finished'][0][i] or pred_type == 'baseline') and valid_i:
        ...
        if already_finished[-1] & aux['valid'][0][-1]:
          ...
          program_outputs = 'N/A'
            ...
            best_program_prediction, functionally_correct, program_outputs = "None", False, "None"

        pt = aux['programs'][0][-1] if _DATASET_TYPE.value == 'deepcoder' else program_parts[0][-1]
        ...
        # Fallback to ExeDec
        # Once we used transductive guidance, update the outer_specs to the output of the first program step, i.e., the step that had transductive guidance
        """ if pred_type == 'tiips' and inner_step_index == 0 and outer_step_index > 0:  #for TIIPS, make sure that for each inner loop, only the first step has transductive guidance
          outer_aux = copy.deepcopy(aux)"""
          # aux['step_outputs'] = outer_aux['step_outputs']
        inner_step_index += 1
      ...
      if s: # if a succesful program was generated in the inner loop, stop the guidance loop
        break
      elif pred_type == 'separate' or pred_type == 'baseline':  # update outer_aux to contain the current program step
        outer_aux = copy.deepcopy(aux)
      elif pred_type == 'tiips' and outer_step_index < outer_step_limit - 1:  # pure inductive program did not solve the task -> transductively guide TIIPS for the next step
        # TODO: after predicting the transductive guidance for the first time execute the first program step to update the outer_specs 
        # Run the SpecDecomposerModel.
        start_time = timeit.default_timer()
        predicted_spec_parts, scores, outer_aux = spec_decomposer_pred_step(  # pylint: disable=undefined-variable
            params=spec_decomposer_optimizer.params,  # pylint: disable=undefined-variable
            inputs=outer_aux['current_inputs'],
            outputs=outer_aux['current_outputs'],
            cache=None,
        ...
        spec_parts_batch = np.array(predicted_spec_parts)
        results = [
            split_outputs(beam, outer_aux['current_outputs'][0][i],
                          max_target_length=_MAX_IO_LENGTH.value, aux=outer_aux,
                          beam_i=i)
            for i, beam in enumerate(spec_parts_batch[0])]
        # split_outputs returns (valids, last_steps, step_outputs,
        # current_outputs)
        ...
        outer_aux['current_outputs'] = jnp.array(current_outputs)[None, Ellipsis]
        outer_aux['step_outputs'] = jnp.array(step_outputs)[None, Ellipsis]
        outer_aux['scores'] = scores
        outer_aux['valid'] = jnp.array(valids)[None, Ellipsis]
        outer_aux['last_step'] = jnp.array(last_steps)[None, Ellipsis]
        current_history_ids = jnp.array(
            [next_history_id() for _ in range(beam_size)])[None, Ellipsis, None]
        outer_aux['history'] = jnp.concatenate([outer_aux['history'], current_history_ids],
                                         axis=-1)

        # Process invalid states.
        if _DETECT_INVALID.value:
          outer_aux['scores'] += decode.NEG_INF * (1 - outer_aux['valid'])
          outer_aux['finished'] |= ~outer_aux['valid']
        spec_processing_times.append(timeit.default_timer() - start_time)

        # Analysis and logging.
        start_time = timeit.default_timer()
        assert len(outer_aux['step_outputs']) == 1
        best_spec_prediction = [decode_spec(o)
                                for o in outer_aux['step_outputs'][0][-1]]
        if outer_aux['finished'][0][-1]:
          best_spec_prediction = '[finished]'
        if outer_aux['finished'][0][-1] & outer_aux['valid'][0][-1]:
          matches = 'N/A'
        else:
          matches = best_spec_prediction == ground_truth_output_parts
          if not matches and first_error is None:
        ...
        if do_logging:
          ...
          for i in range(beam_size)[::-1][:_NUM_BEAM_ELEMENTS_TO_LOG.value]:
            prediction_i = [decode_spec(o) for o in outer_aux['step_outputs'][0][i]]
            score_i = outer_aux['scores'][0][i]
            current_inputs_i = [decode_spec(o)
                                for o in outer_aux['current_inputs'][0][i]]
            current_outputs_i = [decode_spec(o)
                                 for o in outer_aux['current_outputs'][0][i]]
            _, program_i = process_and_decode_program(outer_aux['programs'][0][i])
            valid_i = outer_aux['valid'][0][i]
            last_step_i = outer_aux['last_step'][0][i]
            finished_i = outer_aux['finished'][0][i]
            log_message += (
                f'Beam item {i}:\n'
                f'  prediction: {prediction_i}\n'
                f'  score: {score_i:.4f}\n'
                ...
                f'  program: {program_i}\n'
                f'  valid: {valid_i}, last_step: {last_step_i}, finished: {finished_i}\n'
                f'  history: {outer_aux["history"][0][i]}\n'
            )
        # Program generation step
        # Run the SynthesizerModel.
        start_time = timeit.default_timer()
        synthesizer_step_outputs = outer_aux['step_outputs']
        predicted_program_parts, scores, outer_aux = synthesizer_pred_step(
            params=synthesizer_optimizer.params,
            inputs=outer_aux['current_inputs'],
            outputs=synthesizer_step_outputs,
            cache=None,
        ...
        start_time = timeit.default_timer()
        program_parts = jnp.array([
                np.zeros_like(beam) if outer_aux['finished'][0][i] else beam
                for i, beam in enumerate(np.array(predicted_program_parts)[0])
            ])[None, Ellipsis]
        var_idx = outer_step_index + deepcoder_num_inputs
        outer_aux['programs'] = join_programs(program_parts, outer_aux['programs'],
                                        step=var_idx, num_input_vars=deepcoder_num_inputs) 
        ...
        programs = collections.defaultdict(list)  # Keep track of unique programs.
        for i in range(beam_size):
          valid_i = bool(outer_aux['valid'][0][i])
          last_step_i = outer_aux['last_step'][0][i]
          current_inputs_i = outer_aux['current_inputs'][0][i]
          current_outputs_i = outer_aux['current_outputs'][0][i]

          # Don't need to do any checking if the beam element is already finished
          # or already invalid.
          if (not outer_aux['finished'][0][i] or pred_type == 'baseline') and valid_i:
            # This is just a syntax check.
            valid_i, program_i = process_and_decode_program(outer_aux['programs'][0][i])
            if _USE_EXECUTION.value:
              # Check for syntax, runtime errors, and output prefix matching.
              valid_i, program_outputs_i = run_program(program_i, decoded_inputs)
              success_i = program_outputs_i == decoded_outputs
              ...
              var_idx = outer_step_index + deepcoder_num_inputs
              current_inputs_i = join_inputs(
                  outer_aux['step_outputs'][0][i], current_inputs_i,
                  max_target_length=_MAX_IO_LENGTH.value,
                  variable_index=var_idx)
        ...
        outer_aux['valid'] = jnp.array(new_valids)[None, Ellipsis]
        outer_aux['last_step'] = jnp.array(new_last_steps)[None, Ellipsis]
        outer_aux['current_inputs'] = jnp.array(new_current_inputs)[None, Ellipsis]
        outer_aux['current_outputs'] = jnp.array(new_current_outputs)[None, Ellipsis]
        if s:
          outer_step_index += 1
          break

        already_finished = outer_aux['finished'][0]
        outer_aux['finished'] |= outer_aux['last_step']

        if _DETECT_INVALID.value:
          outer_aux['scores'] += decode.NEG_INF * (1 - outer_aux['valid'])
          outer_aux['finished'] |= ~outer_aux['valid']
        else:
          assert np.all(outer_aux['valid'])
        synthesis_processing_times.append(timeit.default_timer() - start_time)

        # Analysis and logging.
        start_time = timeit.default_timer()

        if already_finished[-1] & outer_aux['valid'][0][-1]:
          best_program_prediction = '[already finished]'
          functionally_correct = 'N/A'
          program_outputs = 'N/A'
        else:
        ...
        pt = outer_aux['programs'][0][-1] if _DATASET_TYPE.value == 'deepcoder' else program_parts[0][-1]
        _, bpp = process_and_decode_program(pt)
        _, po = run_program(bpp, decoded_inputs)
        if po == ground_truth_output_parts or s:
            synth_correct_subgoals += 1
        ...
        synthesis_analysis_times.append(timeit.default_timer() - start_time)
      """else:
        raise ValueError(f'No such prediction type as {pred_type}.')"""
    ...
    for i in reversed(range(beam_size)):
      ...
      if do_logging:
        log_message += (
            ...
            f'  success: {success_i}, score: {outer_aux["scores"][0][i]:.4f}\n'
        )
      if s:
        success = True
        # solution_program = program_i
    ...
    if do_logging:
      log_message += (
          ...
          f'num steps possible:        {outer_step_limit}\n'
    ...
    if pred_type == 'tiips':
      logging.info(f'Test example {test_example_index} resulted in {f"success" if success else "failure"} using {outer_step_index} guidance steps with {solution_program}')
    else:
      logging.info(f'Test example {test_example_index} resulted in {f"success" if success else "failure"} with {solution_program}')
    result_json.append({
        ...
        'num_transductive_guidances': outer_step_index + 1 if pred_type == 'separate' else outer_step_index,  # ExeDec always has at least one guidance step
